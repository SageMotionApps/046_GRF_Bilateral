...
class InertialNet(nn.Module):
    def __init__(self, x_dim, net_name, seed=0, nlayer=1):
        ...
        self.rnn_layer = nn.GRU(
            x_dim, globals()["lstm_unit"], nlayer, batch_first=True, bidirectional=True
        )
    ...
    def forward(self, sequence, lens):
        sequence = pack_padded_sequence(
            sequence, lens, batch_first=True, enforce_sorted=False
        )
...
class OutNet(nn.Module):
    ...
    def __init__(
        self, input_dim, device, output_dim=6, high_level_locs=[2, 3, 4]
    ):  # Changed output_dim to 6
        super(OutNet, self).__init__()
        self.high_level_locs = high_level_locs
        self.linear_1 = nn.Linear(
            input_dim + len(high_level_locs), globals()["fcnn_unit"], bias=True
        ).to(device)
        self.linear_2 = nn.Linear(globals()["fcnn_unit"], output_dim, bias=True).to(
            device
        )
...
class LmfImuOnlyNet(nn.Module):
    def __init__(self, acc_dim, gyr_dim):
        ...
        self.acc_factor = Parameter(
            torch.Tensor(self.rank, 1, 2 * globals()["lstm_unit"] + 1, self.fused_dim)
        ).to(self.device)
        self.gyr_factor = Parameter(
            torch.Tensor(self.rank, 1, 2 * globals()["lstm_unit"] + 1, self.fused_dim)
        ).to(self.device)
